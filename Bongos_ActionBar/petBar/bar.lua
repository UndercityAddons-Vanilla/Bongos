...
local function CreateConfigMenu(name)
	...
	rowsSlider:SetScript("OnValueChanged", function()
		if not this:GetParent().onShow then
			...
			Layout(frame, this:GetValue(), frame.sets.space)
		end
...
local function ShowMenu(bar)
	...
	local sets = BBar.GetSettings(bar.id)
	BongosPetBarMenuHide:SetChecked(not sets.vis)
	BongosPetBarMenuSpacing:SetValue(sets.space or DEFAULT_SPACING)
	BongosPetBarMenuRows:SetValue(sets.rows or DEFAULT_ROWS)

	BongosPetBarMenuScale:SetValue(bar:GetScale() * 100)
	BongosPetBarMenuOpacity:SetValue(bar:GetAlpha() * 100)

	BMenu.ShowForBar(BongosPetBarMenu, bar)
