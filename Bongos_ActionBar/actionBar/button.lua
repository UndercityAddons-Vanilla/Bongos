...
BActionButton = {
	...
	OnClick = function()
		...
		if BActionSets_IsQuickMoveKeyDown() or bg_showGrid then
			...
			local selfCast
			if arg1 == "RightButton" and BActionSets_RightClickSelfCasts() then
				selfCast = 1
			end
	...
	--OnUpdate
	OnUpdate = function()
		if not getglobal(this:GetName() .. "Icon"):IsShown() then return end
...
ActionButtonUp = function(id, onSelf)
	...
	--asian locale fix, there seems to be some issues with button triggering or something
