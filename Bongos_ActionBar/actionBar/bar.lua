...
local function CreateConfigMenu(name)
	...
	local hideButton = CreateFrame("CheckButton", name .. "Hide Hotkeys", menu, "BongosHideBarHotkeysTemplate")
	...
	getglobal(name .. "RowsText"):SetText(BONGOS_ROWS)
...
local function ShowMenu(bar)
	...
	BMenu.ShowForBar(BongosActionBarMenu, bar)
...
local function OnShow() BActionBar.OnShow(this) end
...
BActionBar = {
	...
	Create = function(id)
		...
		if id == 1 and not BActionSets[1] then
			BActionSets[1] = {
				vis = 1,
				paging = 1,
			}
		...
		local bar = BBar.Create(id, "BActionBar" .. id,  "BActionSets." .. id, ShowMenu, nil, OnDelete)
		bar:SetID(id)
	...
	OnShow = function(bar)
		if not getglobal("BActionButton" .. BActionBar.GetStart(bar.id)) then
			Layout(bar.id, bar.sets.rows, bar.sets.space, bar.sets.size)
		end
	...
	GetSize = function(barID)
		return (BActionSets[barID] and BActionSets[barID].size) or floor(MAX_BUTTONS / BActionBar.GetNumber())
	end,
	...
	SetStanceOffset = function(barID, offset)
		--don't do anything if we're already on the right page
		if stanceOffset[barID] and stanceOffset[barID] == offset then return end
	...
	SetContextOffset = function(barID, offset)
		if contextOffset[barID] and contextOffset[barID] == offset then return end
